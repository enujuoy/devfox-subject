...
import com.study.board.domain.dto.UserCntDto;
...
public class UserService {
    ...
    public BindingResult editValid(UserDto dto, BindingResult bindingResult, String loginId)
    {
        ...
        if (dto.getNowPassword().isEmpty()) {
            bindingResult.addError(new FieldError("com/study/board/dto", "nowPassword", "現在のパスワードが入力されていません。"));
        } else if (!encoder.matches(dto.getNowPassword(), loginUser.getPassword())) {
            bindingResult.addError(new FieldError("com/study/board/dto", "nowPassword", "現在のパスワードが間違っています。"));
        }

        if (!dto.getNewPassword().equals(dto.getNewPasswordCheck())) {
            bindingResult.addError(new FieldError("com/study/board/dto", "newPasswordCheck", "パスワードが一致しません。"));
        }

        if (dto.getNickname().isEmpty()) {
            bindingResult.addError(new FieldError("com/study/board/dto", "nickname", "ニックネームが入力されていません。"));
        } else if (dto.getNickname().length() > 10) {
            bindingResult.addError(new FieldError("com/study/board/dto", "nickname", "ニックネームが10文字を超えています。"));
        } else if (!dto.getNickname().equals(loginUser.getNickname()) && userRepository.existsByNickname(dto.getNickname())) {
            bindingResult.addError(new FieldError("com/study/board/dto", "nickname", "ニックネームが重複しています。"));
        }
